...
# logging options
parser.add_argument("--wandb", action="store_true", help="Use wandb to log results")
...
def main():
    ...
    configs = vars(opt)
    configs['model_type'] = 'original'

    # start wandb log if requested
    if opt.wandb:
        import wandb
        wandb.init(
            project="WRN-demo",
    ...
    def on_end_epoch(state):
        ...
        if opt.wandb:
            wandb.log(log_results_dict)
        print(log(log_results_dict, state))
